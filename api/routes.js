...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.status(course ? 200 : 404).json(course ? course : { message: 'Course Not Found' });
  })
);
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (course) {
      const isOwner = await Course.scope('isOwner', {
        method: ['isOwner', req.params.id, req.currentUser.id]
      }).findOne();
